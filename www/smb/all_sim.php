...
if(isset($_GET['action'])) {
	...
	elseif($action=="savemodify")
	{
		...
		if($dev_disable!=$sim_rs['dev_disable']){
			if($dev_disable==0) $db->query("insert into logs set sim_name='$sim_name', log='enabled manually by single sim'");
			else $db->query("insert into logs set sim_name='$sim_name', log='disabled manually by single sim'");
		};
	...
	elseif($action=="savemoremodify")
	{
		...
		if($ErrMsg!="")
			...
			if($_POST['imei_mode_modify']) $sql.="imei_mode='$imei_mode',";
                        if($_POST['month_limit_time_modify']) $sql.="month_limit_time='$_POST[month_limit_time]',month_remain_time='$month_remain_time',month_last_reset_time=now(),";
			...
			if($_POST['dev_disable_modify']){
				...
				while($row=$db->fetch_array($query)) {
					if($dev_disable==0) $db->query("insert into logs set sim_name='$row[sim_name]', log='enabled batch manually'");
					else $db->query("insert into logs set sim_name='$row[sim_name]', log='disabled batch manually'");
				}
			};
			...
			while($row=$db->fetch_array($query)) {
				if($_POST['month_reset_day_modify']) $db->query("insert into logs set sim_name='$row[sim_name]', log='change month limit'");
				sim_info($row, $send);
/*
				if($row['imei']==NULL) $row['imei'] = "000000000000000";
				$imei_len=strlen($row['imei']);
...
*/
		...
		$db->query("update sim set no_ring_remain=0,no_answer_remain=0,short_call_remain=0,remain_time=time_limit,remain_sms=limit_sms,count_remain=count_limit,no_connected_remain=no_connected_limit,month_remain_time=if(month_limit_time=-1, -1, month_limit_time*60) $where");

		$query=$db->query("select sim.*,password from sim left join sim_bank on sim.bank_name=sim_bank.name $where");
		while($row=$db->fetch_array($query)) {
			$db->query("insert into logs set sim_name='$row[sim_name]', log='reset limit from web'");
			sim_info($row, $send);
/*
			if($row['imei']==NULL) $row['imei'] = "000000000000000";
			$imei_len=strlen($row['imei']);
...
*/
		...
		while($row=$db->fetch_array($query)) {
			$db->query("insert into logs set sim_name='$row[sim_name]', log='reset period limit from web'");
			$send[]=my_pack2(RESET_LIMIT, $row['sim_name']);
		}
		sendto_xchanged($send);
...
*/
		...
		$query=$db->query("SELECT sim.*,sim_team.sim_team_name,sim.line_name,gsm_status FROM sim left join sim_team on sim.sim_team_id=sim_team.sim_team_id left join device_line on device_line.line_name=sim.line_name $where $orderby LIMIT $start_limit,$perpage");
		while($row=$db->fetch_array($query)) {
...
*/
			...
			if($row['month_remain_time']!=-1 && $row['month_remain_time']!=0){
				if($row['month_remain_time'] < 60) $row['month_remain_time'] = 1;
				else $row['month_remain_time'] = floor($row['month_remain_time']/60);
			}
